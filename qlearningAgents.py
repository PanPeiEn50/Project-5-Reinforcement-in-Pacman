...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not legalActions:  # Checks if the list of legal actions is empty
          return 0.0

        # Compute the maximum Q-value among all legal actions
    ...
    def computeActionFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        legalActions = self.getLegalActions(state)
        if not legalActions:  # If there are no legal actions, return None
          return None

        # Use max with a key function to find the action that yields the highest Q-value
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Retrieve the current Q-value for the state and action
        current_q_value = self.getQValue(state, action)

        # Compute the value of the next state
        next_state_value = self.getValue(nextState) if nextState else 0

        # Calculate the updated Q-value using the Q-learning formula
        updated_q_value = ((1 - self.alpha) * current_q_value +
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            print(f"Number of training episodes: {self.numTraining}")
            print("======= Feature Weights =======")
            for feature, weight in self.weights.items():  # Assuming self.weights is a dictionary
